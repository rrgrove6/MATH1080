...
\huge In-class Quiz \#15 \\ % The assignment title
\horrule{2pt} \\[0.5cm] % Thick bottom horizontal rule
}

\author{Date:} % The due date

\date{\normalsize April 8, 2016} % A custom date
...
\newpage
\begin{enumerate}
\item (10 points)  Consider the polar equation, $r = 2 \cos (2 \theta), 0 \le \theta \le 2 \pi$, that describes a rose.
\begin{enumerate}
\item Find all values of $\theta$ for which the curve intersects the pole (origin).
\vspace{2in}
\item Sketch the curve.
\vspace{2.5in}
\item Find the exact area inside \textbf{one} leaf of the rose.
\end{enumerate}
